...
void lightModeForSensor() {
  if(400 < avgLightVoltage) {
    Serv_Rotate(10.0); // Servo open, to light throught
    Set_Led(1);
    display("Temprature: ");
  } else {
    Set_Led(0);
...
void newTimeConfig()
{
  ...
  time.decimalPointer = 0;
  head = (LinkedList*)malloc(sizeof(LinkedList));
  linkedListLength = 0;      // A length of 1440 means 1 day of measuring has been achieved
  measurementCount = 0;      // Used as total sample count to calculate the average temprature for a day
  whatDayInStatsPointer = 0; // Keep track of what day it is when measuring samples
  sevenDaysHasPassed    = 0; // 0 if false
  totalMeasuredTemp = 0;
  clockCounter = 1;
  secSinceLastTempMeasurement = 0;
  isFirstTimeMeasured = 0;
  fastMode = 0; // fastMode = false,
  o = 79;
